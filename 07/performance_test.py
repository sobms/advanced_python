    ...
    for i in range(1, len(args)):
        ...
        for j in range(len(matrix_a)):
            ...
            for k in range(len(matrix_b[0])):
                row.append(sum([matrix_a[j][t] * matrix_b[t][k] 
...
def main():
    args = ([[(3*i+j)/10 for i in range(100)] for j in range(50)], 
    ...
    end_time = time.time()
    print(f"Time of execution of python matrix multiplication is {end_time-start_time} seconds")
    print("+---------- capi ----------+")
    start_time = time.time()
    result_capi = cutils.matrix_multiplication(*args)
    end_time = time.time()
    print(f"Time of execution of capi matrix multiplication is {end_time-start_time} seconds")
