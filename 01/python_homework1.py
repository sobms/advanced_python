...
class TicTacGame:
    ...
    def check_winner(self):  # test
        ...
        if all(all(row) for row in self.field):
            winner = 'End of game'
        if all(self.field[i][i] == self.field[0][0]
               for i in range(dim)) and self.field[0][0] != 0:
            winner = self.markers[self.field[0][0]]
        if all(self.field[dim - 1 - i][i] == self.field[dim - 1][0]
               for i in range(dim)) and self.field[dim - 1][0] != 0:
            winner = self.markers[self.field[dim - 1][0]]
        for i in range(dim):
            if all(self.field[i][j] == self.field[i][0]
                   for j in range(dim)) and self.field[i][0] != 0:  # i-masая строка
                winner = self.markers[self.field[i][0]]
            if all(self.field[j][i] == self.field[0][i]
                   for j in range(dim)) and self.field[0][i] != 0:  # i-ый столбец
